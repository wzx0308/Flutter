...
class BrowseHistoryController extends GetxController {
  ...
  Future<void> loadHistory() async {
    ...
    hasMore.value = true;
    try {
      final res = await _bookmarkProvider.getMyHistory(page: 1, pageSize: 20);
      if (res['code'] == 0) {
        final data = res['data'];
  ...
  Future<void> loadMore() async {
    ...
    try {
      _page++;
      final res = await _bookmarkProvider.getMyHistory(page: _page, pageSize: 20);
      if (res['code'] == 0) {
        final data = res['data'];
        final list = data is Map ? (data['list'] ?? []) : data;
  ...
  Future<void> clearHistory() async {
    try {
      await _bookmarkProvider.clearHistory();
      posts.clear();
      hasMore.value = false;
    } catch (_) {}
  }
